...
const base = (id, type, x, y, width, height, index, colors) => ({
  ...
  boundElements: [], updated: 1783699200000, link: null, locked: false, index: `a${index}`
});
...
const manualRows = {
  "10_structural_parent_child_flow.mmd": [
    ["A"], ["B"], ["C", "G", "BA", "L", "P"], ["D", "H", "BB", "M", "Q"],
    ["E", "I", "BC", "N", "R"], ["F", "J", "BD", "O", "S"], ["K"], ["T"], ["U"],
    ["V"], ["W", "X"], ["Y"], ["Z"]
  ]
};
...
function layout(name, nodes, edges, orientation) {
  ...
  if (manualRows[name]) {
    ...
    const side = { AA: [1900, 790, 3], AB: [1900, 1000, 4], AC: [1900, 1210, 5] };
    for (const [id, [x, y, rank]] of Object.entries(side)) if (nodes.has(id)) positions.set(id, { x, y, rank });
    return positions;
  }
...
function convert(name, source) {
  ...
  for (const [edgeIndex, edge] of edges.entries()) {
    ...
    const routed = route(a, b, orientation, edgeIndex, bounds, edge.dotted, boxes, edge, usedSegments); const { start, end } = routed;
    const arrow = base(`edge-${index}`, "arrow", start[0], start[1], end[0] - start[0], end[1] - start[1], index++, ["#475569", "transparent"]);
    Object.assign(arrow, { backgroundColor: "transparent", strokeStyle: edge.dotted ? "dashed" : "solid",
      points: routed.points, lastCommittedPoint: null,
      startBinding: null, endBinding: null, startArrowhead: null, endArrowhead: "arrow", elbowed: true,
      fixedSegments: routed.points.slice(1, -2).map((point, i) => ({ index: i + 2, start: point, end: routed.points[i + 2] })),
