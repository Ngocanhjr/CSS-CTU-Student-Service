...
AGENTIC_TIERS = {"agentic", "agentic_plus"}
...
def load_custom_prompt() -> str:
    """Đọc prompt chuẩn dùng trên web LlamaParse."""
...
def build_parse_kwargs(file_id: str, options: RawParseOptions) -> dict[str, Any]:
    ...
    if options.tier not in AGENTIC_TIERS:
        raise ValueError(
            f"tier không được hỗ trợ: {options.tier}; dùng agentic hoặc agentic_plus"
        )
    ...
    kwargs["agentic_options"] = {"custom_prompt": load_custom_prompt()}
