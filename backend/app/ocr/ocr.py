...
from .engines.llamaparse_engine import (
    LlamaParseRawEngine,
    RawParseOptions,
    supported_input_files,
)

from .postprocess.llamaparse_postprocess import postprocess_llamaparse_markdown
from .postprocess.page_formatter import render_pages
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="agentic",
        help="LlamaParse agentic tier, mặc định: agentic",
    )
    ...
    parser.add_argument("--language", default="vi", help="Ngôn ngữ OCR, mặc định: vi")
...
def process_input_file(
    ...
    final_output = ap_dung_va_xac_thuc_metadata(
        markdown=processed_output,
        output_path=out_path,
        source_file=file_path,
        language=options.language or "vi",
        ocr_status="done",
        document_type=None,
        parser=parser_name,
...
def main() -> int:
    ...
    if not os.getenv("LLAMA_CLOUD_API_KEY"):
        raise RuntimeError("Thiếu biến môi trường LLAMA_CLOUD_API_KEY.")
