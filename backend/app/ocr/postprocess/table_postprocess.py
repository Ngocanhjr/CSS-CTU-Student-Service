...
def is_table_separator(line: str) -> bool:
    ...
    return len(cells) >= 2 and all(re.fullmatch(r"\s*:?-{2,}:?\s*", cell or "") for cell in cells)


def split_table_row(line: str) -> list[str]:
    """Tách Markdown row, kể cả bảng OCR thiếu một hoặc hai dấu `|` ngoài."""
    ...
    if s.startswith("|") and s.endswith("|"):
        ...
        s = s[:-1]
    # `| nội dung` trong bảng loose hai cột biểu diễn ô đầu rỗng.
    return [cell.strip() for cell in s.split("|")]


def is_loose_table_line(line: str) -> bool:
    """Nhận diện row có pipe nhưng thiếu outer pipes do OCR."""
...
def normalize_single_table(block: list[str]) -> list[str]:
    ...
    rows = [
        [re.sub(r"^\\+\*(?=\s)", "•", cell) for cell in split_table_row(line)]
        for line in block
    ]
    if not rows:
        return block
...
def normalize_tables(markdown: str) -> str:
    ...
    while index < len(lines):
        starts_loose_table = (
            is_loose_table_line(lines[index])
            and index + 1 < len(lines)
            and is_table_separator(lines[index + 1])
        )
        if not is_table_line(lines[index]) and not starts_loose_table:
        ...
        while index < len(lines) and (
            is_table_line(lines[index]) or is_loose_table_line(lines[index])
        ):
            index += 1
        out.extend(normalize_single_table(lines[start:index]))
...
TAG_RE = re.compile(r"<[^>]+>")
PAGE_COMMENT_RE = re.compile(r"<!--\s*page\s*:\s*\d+\s*-->", flags=re.IGNORECASE)
...
def fix_html_table_page_continuations(markdown: str) -> str:
    ...
    Hàm này không gộp các bảng của nhiều trang thành một `<table>` duy nhất.
    Mục tiêu là giữ output
...
def parse_html_table_rows(section_html: str) -> list[list[str]]:
    ...
    for tr_match in TR_RE.finditer(section_html):
        ...
        def flush_pending() -> None:
            """Đổ các cell rowspan đang chờ vào row hiện tại."""
...
def convert_html_tables_to_markdown(markdown: str) -> str:
    ...
    for match in matches:
        gap = markdown[pos : match.start()]
        table_html = match.group(0)
        out.append(gap)
        inherited_header = last_header if PAGE_COMMENT_RE.search(gap) else None
        md_table, last_header = html_table_to_markdown(table_html, inherited_header=inherited_header)
        out.append(md_table)
        pos = match.end()
