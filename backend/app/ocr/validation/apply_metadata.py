...
BACKEND_DOCUMENT_TYPES = set(get_args(DocumentType))
BACKEND_OCR_STATUSES = set(get_args(OcrStatus))
BACKEND_REVIEW_STATUSES = set(get_args(ReviewStatus))
BACKEND_RAG_STATUSES = set(get_args(RagStatus))
BACKEND_FILE_TYPES = set(get_args(FileType))
BACKEND_AUDIENCES = set(get_args(Audience))
BACKEND_DOMAINS = set(get_args(Domain))
...
def canonical_markdown_path(source_path: Path | None, md_path: Path) -> str:
    ...
    với output thật. Ở bước OCR, ocr.py đã quyết định output_path chính xác,
    ...
    metadata.update(
        {
            ...
            "domain": normalize_enum(existing.get("domain"), BACKEND_DOMAINS, "unknown"),
            ...
            "audience": existing.get("audience") if not is_empty(existing.get("audience")) else ["sinh_vien"],
...
def ap_dung_va_xac_thuc_metadata(
    ...
    **kw,
    ...
    Dùng chung ở ocr.py, hybrid_page_router, hybrid_router và llamaparse_engine
